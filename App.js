...
async function generateMeals(e) {
    ...
    mealData = caloryData.find(data => +data.min <= calories && +data.max >= calories);
    ...
    breakfastBox.querySelector('.calories').textContent = `Calories - ${calories}`;
    ...
    const divActive = document.querySelector('.div-active').classList.add('active');
}
